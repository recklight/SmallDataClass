...
source_sub = [
    tf.data.Dataset.list_files(os.path.join(sc, '*.JPG'), shuffle=True).map(map_fun) for sc in source_classes]
target_sub = [
    tf.data.Dataset.list_files(os.path.join(sc, '*.JPG'), shuffle=True).map(map_fun) for sc in target_classes]
